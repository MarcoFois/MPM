...
/*
 Aggiornamento velocita' particelle con  FLIP/PIC:
   vpx = alpha*(vpx_old + dt*a) + (1-alpha)*v_grid
 La posizione e' advettata con la velocita' di griglia (PIC), piu' stabile.
 Occhio a non abbassare troppo alpha. Diventa molto instabile. Usalo con alpha = 0.95/0.96
 */
...
int main ()
{
  ...
  // --- Riporta le particelle dentro i confini --------------------------------
  double xmax = grid.hx() * grid.num_cols();
  double ymax = grid.hy() * grid.num_rows();
  double tol  = 0.1;
  for (size_t ip = 0; ip < num_particles; ++ip) {
    if (std::abs(ptcls.x[ip]-xmax) < tol/2 || ptcls.x[ip] > xmax) ptcls.x[ip] = xmax - tol;
    if (ptcls.x[ip] < 0.)                                          ptcls.x[ip] = 0.;
    if (std::abs(ptcls.y[ip]-ymax) < tol/2 || ptcls.y[ip] > ymax) ptcls.y[ip] = ymax - tol;
    if (ptcls.y[ip] < 0.)                                          ptcls.y[ip] = 0.;
  }

  ptcls.init_particle_mesh ();

  // --- Azzeramento proprieta' di lavoro --------------------------------------
  ...
  // Peso del blend FLIP/PIC: 1.0 = FLIP puro (poco dissipativo, piu' rumoroso),
  // 0.0 = PIC puro - occhio che è molto disspativo
  const double flip_alpha = 0.95;


  const double cfl = 0.09;
  ...
  const double save_interval_s = 0.25;
  const double save_time_eps = 1.e-4;
  double next_save_time = 0.0;
  auto format_save_time = [] (double time_value) {
    std::string s = std::to_string (time_value);
    // Rimuove le cifre decimali di troppo
  ...
  while (t < data.T) // data.T
    {
      ...
      if (it > 0)
        dt = cfl * data.hx / (1e-2 + cel);
      std::cout << "time = " << t << "   dt = " << dt << "   cel = " << cel << std::endl;
      my_timer.toc ("update dt");

      // ----- output csv ------------------------------------------------------
      my_timer.tic ("save csv");
      if (t + save_time_eps >= next_save_time)
        {
          std::string filename = "nc_particles_" + format_save_time (next_save_time) + "s.csv";
#ifdef USE_COMPRESSION
      filename = filename + ".gz";
          ...
          OF.close ();
#endif
next_save_time += save_interval_s;
        }
      ++it;
      ...
      transform (policy, ptcls.dprops["Ap"].begin (), ptcls.dprops["Ap"].end (),
                 ptcls.dprops["Fb_x"].begin (), ptcls.dprops["Fric_px"].begin (), std::multiplies<double> ());
      transform (policy, ptcls.dprops["Ap"].begin (), ptcls.dprops["Ap"].end (),
                 ptcls.dprops["Fb_y"].begin (), ptcls.dprops["Fric_py"].begin (), std::multiplies<double> ());
      ...
      // (3) FORZE INTERNE (p2gd) e BILANCIO DI QUANTITA' DI MOTO --------------
      ...
      // (4) ACCELERAZIONI E VELOCITA' NODALI ----------------------------------
      my_timer.tic ("step 4");
      ...
      // (6) RITORNO ALLE PARTICELLE (G2P) - VELOCITA' E POSIZIONE -------------
      //
      //   Blend FLIP/PIC
      //   NB: NON si azzerano vpx/vpy (servono come velocita' "vecchia" FLIP).
      ...
      ptcls.dprops.at("vpx_dy").assign(ptcls.num_particles, 0.0);// Azzerati anche questi che sono usati per lo stress tensor
      ptcls.dprops.at("vpy_dx").assign(ptcls.num_particles, 0.0);
      my_timer.toc ("step 6");

      my_timer.tic ("g2p");
      ptcls.g2p (vars, {"avx","avy","vvx","vvy"}, {"apx","apy","vpxL","vpyL"});
      my_timer.toc ("g2p");
      ...
      // FLIP: vpx <- vpx_old + dt*apx
      transform (policy, ptcls.dprops["vpx"].begin (), ptcls.dprops["vpx"].end (),
                 ptcls.dprops["apx"].begin (), ptcls.dprops["vpx"].begin (),
                 [=] (double v, double a) { return v + dt * a; });
      transform (policy, ptcls.dprops["vpy"].begin (), ptcls.dprops["vpy"].end (),
                 ptcls.dprops["apy"].begin (), ptcls.dprops["vpy"].begin (),
                 [=] (double v, double a) { return v + dt * a; });
      // blend con PIC (vpxL/vpyL)
      transform (policy, ptcls.dprops["vpx"].begin (), ptcls.dprops["vpx"].end (),
                 ptcls.dprops["vpxL"].begin (), ptcls.dprops["vpx"].begin (),
                 [=] (double vflip, double vpic) { return flip_alpha*vflip + (1.0-flip_alpha)*vpic; });
      transform (policy, ptcls.dprops["vpy"].begin (), ptcls.dprops["vpy"].end (),
                 ptcls.dprops["vpyL"].begin (), ptcls.dprops["vpy"].begin (),
                 [=] (double vflip, double vpic) { return flip_alpha*vflip + (1.0-flip_alpha)*vpic; });

      // posizione: advezione con la velocita' di griglia (PIC)
      transform (policy, ptcls.x.begin (), ptcls.x.end (), ptcls.dprops["vpxL"].begin (),
                 ptcls.x.begin (), [=] (double x, double v) { return x + dt * v; });
      transform (policy, ptcls.y.begin (), ptcls.y.end (), ptcls.dprops["vpyL"].begin (),
                 ptcls.y.begin (), [=] (double y, double v) { return y + dt * v; });

      // Mantieni le particelle dentro i confini
      for (size_t ip = 0; ip < num_particles; ++ip) {
        if (std::abs(ptcls.x[ip]-xmax) < tol/2 || ptcls.x[ip] > xmax) ptcls.x[ip] = xmax - tol;
        if (ptcls.x[ip] < 0.)                                          ptcls.x[ip] = 0.;
        if (std::abs(ptcls.y[ip]-ymax) < tol/2 || ptcls.y[ip] > ymax) ptcls.y[ip] = ymax - tol;
      ...
      // (7) ALTEZZA DA DIVERGENZA DI VELOCITA' --------------------------------
      // my_timer.tic ("step 7a");
      // my_timer.toc ("step 7a");

      my_timer.tic ("step 7");
      // div(v) = dvx/dx + dvy/dy
      transform (policy, ptcls.dprops["vpx_dx"].begin (), ptcls.dprops["vpx_dx"].end (),
                 ptcls.dprops["vpy_dy"].begin (), div_vp.begin (), std::plus<double> ());



      // Aggiornamento area: Ap <- Ap * exp(div * dt)
      transform (policy, ptcls.dprops["Ap"].begin(), ptcls.dprops["Ap"].end(),
                 div_vp.begin(), ptcls.dprops["Ap"].begin(),
                 [dt](double a, double d) { return a * std::exp(d * dt); });

      // Clamp di Ap a un minimo fisico
      const double Ap0 = 0.03;
      transform (policy, ptcls.dprops["Ap"].begin(), ptcls.dprops["Ap"].end(),
                 ptcls.dprops["Ap"].begin(),
                 [Ap0](double a) { return std::max(a, Ap0 / 1000.0); });

      // hp = Mp / (rho * Ap)
      transform (policy, ptcls.dprops["Mp"].begin(), ptcls.dprops["Mp"].end(),
                 ptcls.dprops["Ap"].begin(), ptcls.dprops["hp"].begin(),
                 [&](double m, double a) { return m / (data.rho * a); });

      // quantita' di moto particelle aggiornata
      transform (policy, ptcls.dprops["vpx"].begin (), ptcls.dprops["vpx"].end (),
                 ptcls.dprops["Mp"].begin (), ptcls.dprops["mom_px"].begin (), std::multiplies<double> ());
      transform (policy, ptcls.dprops["vpy"].begin (), ptcls.dprops["vpy"].end (),
                 ptcls.dprops["Mp"].begin (), ptcls.dprops["mom_py"].begin (), std::multiplies<double> ());
      my_timer.toc ("step 7");

      // (7b) FRIZIONI (qui disattivate: fattore 0.0) --------------------------
      ...
      transform (policy, ptcls.dprops["vpx"].begin (), ptcls.dprops["vpx"].end (), ptcls.dprops["hp"].begin (), ptcls.dprops["Fb_x"].begin (),
                 [&] (double x, double y) { return 0.0 * data.FRICTION_ON * (data.rho * data.g * y * std::tan(fric_ang) + data.rho * data.g * x * x / data.xi) * x; });
      ...
      // (8) AGGIORNAMENTO SFORZI PARTICELLE (USL) -----------------------------
      my_timer.tic ("step 8");
    ...
    std::string filename2 = "nc_particles_final_instant.csv";
    std::ofstream OF (filename2.c_str ());
    ptcls.print<particles_t::output_format::csv>(OF);
